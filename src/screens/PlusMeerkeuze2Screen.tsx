...
function generateQuestion(id: number) {
  ...
  // Shuffle options
  const options = [
    { sum: `${num1} + ${num2} = ${correct}`, isCorrect: true },
    { sum: `${num1} + ${num2} = ${distractor1}`, isCorrect: false },
    { sum: `${num1} + ${num2} = ${distractor2}`, isCorrect: false },
  ].sort(() => Math.random() - 0.5);
  return {
    id,
    question: 'Welke som is goed?',
    options,
  };
}
...
const PlusMeerkeuze2Screen: React.FC = () => {
  ...
  const handleAnswerSubmit = () => {
    const selectedOption = currentQuestion.options.find(opt => opt.sum === selectedAnswer);
...
export default PlusMeerkeuze2Screen; 